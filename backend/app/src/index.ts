...
app.get("/allTodos", async (req, res) => {
  ...
  try {
    ...
    console.error("allTodos error:", error);
  }
});
...
app.post("/bulkUpdateTodos", async (req, res) => {
  ...
  for (const todo of todos) {
    if (
      ...
      return res
        .status(400)
        .json({ error: "各todoにid, title, description, completed, priority, due_date(ISO8601)が必要です" });
    }
  }
  // due_dateをDate型に変換
  ...
  try {
    await prisma.$transaction([prisma.todo.deleteMany({}), prisma.todo.createMany({ data: todosForDb })]);
