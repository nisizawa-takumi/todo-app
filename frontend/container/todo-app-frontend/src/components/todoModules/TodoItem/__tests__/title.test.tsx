import { render, screen, fireEvent } from "@testing-library/react";
import "@testing-library/jest-dom";
import TodoTitle from "../title";
import type { TodoType } from "../../../../lib/todo/apiClient";
import { TODO_UPDATE_INTERVAL_MS } from "@/constants/timing";
const mockUpdateOne = jest.fn().mockResolvedValue(undefined);

const mockTodo: TodoType = {
  id: "1",
  title: "テストタイトル",
  description: "説明",
  completed: false,
  due_date: "2025-06-20",
  priority: "high",
};

describe("TodoTitle", () => {
  it("タイトルが表示される", () => {
    render(<TodoTitle todoItem={mockTodo} updateOne={mockUpdateOne} variant="outlined" />);
    expect(screen.getByDisplayValue("テストタイトル")).toBeInTheDocument();
  });

  it("タイトルを編集できる (TODO_UPDATE_INTERVAL_MS遅延後に更新)", () => {
    jest.useFakeTimers();
    render(<TodoTitle todoItem={mockTodo} updateOne={mockUpdateOne} variant="outlined" />);
    const input = screen.getByDisplayValue("テストタイトル");
    fireEvent.change(input, { target: { value: "新しいタイトル" } });
    fireEvent.blur(input);
    jest.advanceTimersByTime(TODO_UPDATE_INTERVAL_MS);
    expect(mockUpdateOne).toHaveBeenCalled();
    jest.useRealTimers();
  });

  it("updateOneが失敗した場合にsetErrorが呼ばれる", () => {
    jest.useFakeTimers();
    const errorMessage = "タイトル更新失敗";
    const mockUpdateOneReject = jest.fn().mockRejectedValue(new Error(errorMessage));
    const mockSetError = jest.fn();
    render(
      <TodoTitle todoItem={mockTodo} updateOne={mockUpdateOneReject} setError={mockSetError} variant="outlined" />
    );
    const input = screen.getByDisplayValue("テストタイトル");
    fireEvent.change(input, { target: { value: "異常タイトル" } });
    fireEvent.blur(input);
    jest.advanceTimersByTime(TODO_UPDATE_INTERVAL_MS);
    return Promise.resolve().then(() => {
      expect(mockSetError).toHaveBeenCalledWith(errorMessage);
      jest.useRealTimers();
    });
  });
});
